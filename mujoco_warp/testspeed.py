...
def _main(argv: Sequence[str]):
  ...
  nstep, nworld, nconmax, njmax, njmax_nnz, nccdmax, override, device = (
    _NSTEP.value,
    _NWORLD.value,
    _NCONMAX.value,
    _NJMAX.value,
  ...
  m, d, metrics = mjw.benchmark.init(mjm, mjd, nworld, nconmax, njmax, njmax_nnz, nccdmax, override, device)
  ...
  # RenderContext
  if "rc" in inspect.signature(fn).parameters.keys():
    print("RenderContext")
  ...
  print(f"Rolling out {nstep} steps at dt = {f'{timestep:g}' if timestep < 0.001 else f'{timestep:.3f}'}...")

  fn = _FUNCS[_FUNCTION.value]
  metrics = mjw.benchmark.run(fn, m, d, metrics, nstep, ctrls, _EVENT_TRACE.value, rc, device, _CLEAR_WARP_CACHE.value)

  steps = nworld * nstep
  print(f"""
Summary for {nworld} parallel rollouts

Total JIT time: {metrics.jit_duration:.2f} s
Total simulation time: {metrics.run_time:.2f} s
Total steps per second: {steps / metrics.run_time:,.0f}
Total realtime factor: {steps * timestep / metrics.run_time:,.2f} x
Total time per step: {1e9 * metrics.run_time / steps:.2f} ns
Total converged worlds: {metrics.converged_worlds} / {d.nworld}""")
  ...
  if _MEASURE_ALLOC.value and metrics.nacon and metrics.nefc:
    idx = 0
    nacon_matrix, nefc_matrix = [], []
    for i in range(_NUM_BUCKETS.value):
      size = nstep // _NUM_BUCKETS.value + (i < (nstep % _NUM_BUCKETS.value))
      nacon_arr = np.array(metrics.nacon[idx : idx + size])
      nefc_arr = np.array(metrics.nefc[idx : idx + size])
  ...
  if _MEASURE_SOLVER.value and metrics.solver_niter:
    idx = 0
    matrix = []
    for i in range(_NUM_BUCKETS.value):
      size = nstep // _NUM_BUCKETS.value + (i < (nstep % _NUM_BUCKETS.value))
      arr = np.array(metrics.solver_niter[idx : idx + size])
  ...
  if _MEMORY.value:
    ...
    used_mem = metrics.total_memory
    other_mem = used_mem
    for mem, name in [(metrics.model_mem_breakdown, "\nModel"), (metrics.data_mem_breakdown, "Data")]:
      if mem is None:
        continue
