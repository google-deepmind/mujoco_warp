...
def sphere_capsule(
  ...
  pt = closest_segment_point(
      capsule_pos - segment, capsule_pos + segment, sphere_pos
  )
...
def plane_capsule(
  ...
  dist1, pos1 = plane_sphere(
      n, plane_pos, capsule_pos + segment, capsule_radius
  )

  # Second contact (negative end of capsule)
  dist2, pos2 = plane_sphere(
      n, plane_pos, capsule_pos - segment, capsule_radius
  )
...
def plane_ellipsoid(
  ...
  Returns:
    Tuple containing:
      ...
      contact_normals: Matrix of contact normal vectors (one per row)
  """
  sphere_support = -wp.normalize(
      wp.cw_mul(wp.transpose(ellipsoid_rot) @ plane_normal, ellipsoid_size)
  )
  pos = ellipsoid_pos + ellipsoid_rot @ wp.cw_mul(
      sphere_support, ellipsoid_size
  )
...
def plane_box(
  ...
  Returns:
    Tuple containing:
      contact_dist: Vector of contact distances (wp.inf for unpopulated
      contacts)
      contact_pos: Matrix of contact positions (one per row)
...
def plane_cylinder(
  ...
  vec = wp.where(
      ...
      wp.vec3(1.0, 0.0, 0.0)
      * cylinder_radius,  # Default x-axis when degenerate
  )
...
def box_box(
  ...
  rot21abs = wp.matrix_from_rows(
      wp.abs(rot21[0]), wp.abs(rot21[1]), wp.abs(rot21[2])
  )
  ...
  if axis_code < 12:
    ...
    for i in range(4):
      for q in range(2):
        la = pts_lp[q] + wp.where(
            i < 2, 0.0, wp.where(i == 2, pts_cn1[q], pts_cn2[q])
        )
        ...
        lua = axi_lp + wp.where(
            i < 2, wp.vec3(0.0), wp.where(i == 2, axi_cn1, axi_cn2)
        )
...
def sphere_box(
  ...
  if dist <= MJ_MINVAL:
    ...
    for i in range(6):
      face_dist = wp.abs(
          wp.where(i % 2, 1.0, -1.0) * box_size[i // 2] - center[i // 2]
      )
...
def capsule_box(
  ...
  axisdir = (
      wp.int32(halfaxis[0] > 0.0)
      + 2 * wp.int32(halfaxis[1] > 0.0)
      + 4 * wp.int32(halfaxis[2] > 0.0)
  )
  ...
  dist1, pos1, normal1 = sphere_box(
      s1_pos_g, capsule_radius, box_pos, box_rot, box_size
  )
  ...
  if secondpos > -3:  # secondpos was modified
    ...
    # collide with sphere using core function
    dist2, pos2, normal2 = sphere_box(
        s2_pos_g, capsule_radius, box_pos, box_rot, box_size
    )
  else:
    dist2 = wp.inf
    pos2 = wp.vec3()
