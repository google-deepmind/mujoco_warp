...
def _tree_edges(
  ...
  if use_generic == 0:
    ...
    if tree0 >= 0:
      if tree1 < 0 or tree0 == tree1:
        # self-edge
        wp.atomic_max(edge_seen, worldid, tree0, tree0, 1)
      else:
        # cross-tree edge
        t1 = wp.min(tree0, tree1)
        t2 = wp.max(tree0, tree1)
        wp.atomic_max(edge_seen, worldid, t1, t2, 1)
    return
  ...
  if first_tree >= 0 and has_cross_edge == 0:
    wp.atomic_max(edge_seen, worldid, first_tree, first_tree, 1)


def tree_edges(m: types.Model, d: types.Data, edges: wp.array3d(dtype=int)):
