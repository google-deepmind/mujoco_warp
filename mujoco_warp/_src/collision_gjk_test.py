...
def _geom_dist(
  ...
  # TODO(team): update after put_model sets nmaxpolygon=0, nmaxmeshdeg=0 when no multiccd geom pairs
  if multiccd:
    nmaxpolygon = int(m.nmaxpolygon)
    nmaxmeshdeg = int(m.nmaxmeshdeg)
  else:
    nmaxpolygon = 0
    nmaxmeshdeg = 0
  ...
  def _gjk_kernel(
    ...
    dist, ncon, x1, x2 = wp.static(ccd(multiccd))(
      tolerance[0],
      1.0e30,
      iterations,
      geom1,
