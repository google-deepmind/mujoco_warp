...
def sample_volume_sdf(xyz: wp.vec3, volume_data: VolumeData) -> float:
  ...
  r = xyz - center
  q = wp.vec3(
      wp.abs(r[0]) - half_size[0],
      wp.abs(r[1]) - half_size[1],
      wp.abs(r[2]) - half_size[2],
  )
...
def clearance(
  ...
  sdf2 = sdf(
      int(GeomType.SDF.value), p2, s2, sdf_type2, volume_data2, mesh_data2
  )
  if sfd_intersection:
...
def gradient_descent(
  ...
  grad1 = sdf_grad(
      type1, x_1, params.attr1, sdf_type1, volume_data1, mesh_data1
  )
