...
class IslandEdgeDiscoveryTest(absltest.TestCase):
  ...
  def test_single_constraint_two_trees(self):
    ...
    edges = wp.zeros((d.nworld, m.ntree, m.ntree), dtype=int)
    island.tree_edges(m, d, edges)

    te = edges.numpy()
    self.assertEqual(te[0, 0, 1], 1)  # edge between tree 0 and tree 1
  ...
  def test_constraint_within_single_tree_creates_self_edge(self):
    ...
    te = edges.numpy()
    self.assertEqual(te[0, 0, 0], 1)  # self-edge for tree 0
  ...
  def test_three_bodies_chain(self):
    ...
    te = edges.numpy()
    self.assertEqual(te[0, 0, 1], 1)  # edge (0, 1)
    self.assertEqual(te[0, 1, 2], 1)  # edge (1, 2)
  ...
  def test_deduplication(self):
    ...
    te = edges.numpy()
    self.assertEqual(te[0, 0, 1], 1)  # edge exists
    self.assertEqual(np.sum(te[0]), 1)  # only 1 unique edge
  ...
  def test_no_constraints(self):
    ...
    te = edges.numpy()
    self.assertEqual(np.sum(te[0]), 0)
  ...
  def test_multi_world_parallel(self):
    ...
    te = edges.numpy()
    self.assertEqual(te[0, 0, 1], 1)  # world 0
    self.assertEqual(te[1, 0, 1], 1)  # world 1
  ...
  def test_isolated_tree_no_edge(self):
    ...
    te = edges.numpy()
    self.assertEqual(np.sum(te[0]), 0)
    self.assertEqual(np.sum(te[1]), 0)
  ...
  def test_mixed_equality_and_contact(self):
    ...
    te = edges.numpy()
    self.assertEqual(te[0, 1, 2], 1)  # equality constraint edge (B, C)
  ...
  def test_worldbody_dofs_ignored(self):
    ...
    te = edges.numpy()
    self.assertEqual(te[0, 0, 0], 1)  # self-edge for floating tree
  ...
  def test_constraint_touches_three_trees(self):
    ...
    te = edges.numpy()
    self.assertEqual(te[0, 0, 1], 1)  # edge (A, B)
    self.assertEqual(te[0, 0, 2], 1)  # edge (A, C)
