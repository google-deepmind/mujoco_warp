...
def contact_params(
  ...
  if pairid > -1:
    ...
    solimp = (
        mix * geom_solimp[worldid, g1] + (1.0 - mix) * geom_solimp[worldid, g2]
    )
...
def sphere_sphere_wrapper(
  ...
  dist, pos, normal = sphere_sphere(
      sphere1.pos, sphere1.size[0], sphere2.pos, sphere2.size[0]
  )
...
def sphere_capsule_wrapper(
  ...
  dist, pos, normal = sphere_capsule(
      sphere.pos, sphere.size[0], cap.pos, axis, cap.size[0], cap.size[1]
  )
...
def plane_ellipsoid_wrapper(
  ...
  dist, pos, normal = plane_ellipsoid(
      plane.normal, plane.pos, ellipsoid.pos, ellipsoid.rot, ellipsoid.size
  )
...
def plane_box_wrapper(
  ...
  dist, pos, normal = plane_box(
      plane.normal, plane.pos, box.pos, box.rot, box.size
  )
...
def sphere_cylinder_wrapper(
  ...
  cylinder_axis = wp.vec3(
      cylinder.rot[0, 2], cylinder.rot[1, 2], cylinder.rot[2, 2]
  )
...
def sphere_box_wrapper(
  ...
  dist, pos, normal = sphere_box(
      sphere.pos, sphere.size[0], box.pos, box.rot, box.size
  )
