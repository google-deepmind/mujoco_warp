...
class DerivativeTest(parameterized.TestCase):
  ...
  @parameterized.parameters(
    ...
    (mujoco.mjtJacobian.mjJAC_SPARSE, "ellipsoid"),
  )
  def test_smooth_vel_fluid(self, jacobian, fluidshape):
    """Tests qDeriv with fluid forces (ellipsoid shapes only)."""
    xml = f"""
    <mujoco>
      <option integrator="implicitfast" density="1.0" viscosity="0.001">
        <flag gravity="disable"/>
      </option>
      <worldbody>
        <body>
        ...
        <body pos="1 0 0">
          <geom type="sphere" size=".1" fluidshape="{fluidshape}" fluidcoef="0.5 0.5 0.5 0.5 0.5"/>
          <joint name="joint1" type="hinge" axis="0 1 0"/>
          <site name="site1" pos="0 0 1"/>
        </body>
        <body pos="2 0 0">
    ...
    _assert_eq(mjw_out, mj_out, "qM - dt * qDeriv (with fluid)")
