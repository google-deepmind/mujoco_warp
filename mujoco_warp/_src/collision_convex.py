...
def ccd_hfield_kernel_builder(
  ...
  def ccd_hfield_kernel(
    ...
    epa_index = epa_index_in[ccdid]
    epa_map = epa_map_in[ccdid]
...
def convex_narrowphase(m: Model, d: Data):
  ...
  epa_vert = wp.empty(shape=(d.naccdmax, 5 + epa_iterations), dtype=wp.vec3)
  ...
  epa_face = wp.empty(shape=(d.naccdmax, 6 + MJ_MAX_EPAFACES * epa_iterations), dtype=wp.vec3i)
  ...
  epa_index = wp.empty(shape=(d.naccdmax, 6 + MJ_MAX_EPAFACES * epa_iterations), dtype=int)
  # epa_map: status of faces in polytope
  epa_map = wp.empty(shape=(d.naccdmax, 6 + MJ_MAX_EPAFACES * epa_iterations), dtype=int)
  # epa_horizon: index pair (i j) of edges on horizon
  epa_horizon = wp.empty(shape=(d.naccdmax, 2 * MJ_MAX_EPAHORIZON), dtype=int)
