...
def ccd_kernel_builder(
  ...
  def write_ccd_contacts(
    ...
    witness2: mat43,
    dists: wp.vec4,
    # Pair-local frame origin (geom1.pos at solve time). The CCD solve runs with
    # geom1 translated to the world origin for precision, so witness points come
    # back pair-local and must be shifted by this to be world positions.
    origin: wp.vec3,
    geoms: wp.vec2i,
    worldid: int,
    margin: float,
  ...
  def eval_ccd_write_contact(
    ...
    ccd_origin_out: wp.array[wp.vec3],
  ):
    geom1.margin = margin
    geom2.margin = margin
    ...
    if wp.static(deterministic):
      ...
      ccd_origin_out[collisionid] = origin
      return
  ...
  def ccd_write_kernel(
    ...
    ccd_origin_in: wp.array[wp.vec3],
    # Data out:
    contact_dist_out: wp.array[float],
    contact_pos_out: wp.array[wp.vec3],
    contact_frame_out: wp.array[wp.mat33],
...
def convex_narrowphase(m: Model, d: Data, ctx: CollisionContext, collision_table: list[tuple[GeomType, GeomType]]):
  ...
  # pair-local frame origin per candidate, staged alongside the witness points so
  # the deterministic write kernel can restore world positions from a previous launch
  ccd_origin = wp.empty(nstage, dtype=wp.vec3)
  ...
  for geom_pair in collision_table:
    ...
    if g1 != GeomType.HFIELD and g2 != GeomType.HFIELD and count:
      ...
      wp.launch(
        ...
        outputs=contact_outputs + [d.overflow, ccd_ncollision, ccd_dist, ccd_witness1, ccd_witness2, ccd_dists, ccd_origin],
      )
