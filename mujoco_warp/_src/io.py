...
def _check_flex_key_limits(mjm: mujoco.MjModel, nworld: int):
  ...
  for fi in range(mjm.nflex):
    ...
    # Check nodes
    start_node = mjm.flex_nodeadr[fi]
    end_node = mjm.flex_nodeadr[fi + 1] if fi < mjm.nflex - 1 else mjm.nflexvert
    num_nodes = end_node - start_node
    if num_nodes >= FLEX_KEY_ELEM_LIMIT:
      raise ValueError(f"Flex object {fi} has too many nodes: {num_nodes} (limit is {FLEX_KEY_MAX_ELEM:,}).")
