...
class ActuatorDelayTest(absltest.TestCase):
  ...
  def test_actuator_history_only(self):
    """Test actuator history with delay=0.

    delay=0.0, timestep=0.01, nsample=3.
    ctrl should be applied immediately, but history should be recorded.
    """
    xml = """
    <mujoco>
      <option timestep="0.01"/>
    ...
    wp.copy(d.ctrl, wp.array(np.full((1, 1), 10.0), dtype=float))
    forward.step(m, d)
    np.testing.assert_allclose(d.actuator_force.numpy()[0, 0], 10.0, atol=_TOLERANCE, err_msg="force step 1")

    # Step 2
    wp.copy(d.ctrl, wp.array(np.full((1, 1), 20.0), dtype=float))
    forward.step(m, d)
    np.testing.assert_allclose(d.actuator_force.numpy()[0, 0], 20.0, atol=_TOLERANCE, err_msg="force step 2")

    # Step 3
    wp.copy(d.ctrl, wp.array(np.full((1, 1), 30.0), dtype=float))
    forward.step(m, d)
    np.testing.assert_allclose(d.actuator_force.numpy()[0, 0], 30.0, atol=_TOLERANCE, err_msg="force step 3")

    # Verify history at different times
    result = wp.empty(d.nworld, dtype=float)

    # t = 0.005 (between 0.0 and 0.01)
    time_arr = wp.array([0.005], dtype=float)
    history.read_ctrl(m, d, 0, time_arr, interp=0, result=result)
    np.testing.assert_allclose(result.numpy()[0], 10.0, atol=_TOLERANCE, err_msg="t=0.005")

    # t = 0.015 (between 0.01 and 0.02)
    time_arr = wp.array([0.015], dtype=float)
    history.read_ctrl(m, d, 0, time_arr, interp=0, result=result)
    np.testing.assert_allclose(result.numpy()[0], 20.0, atol=_TOLERANCE, err_msg="t=0.015")

    # t = 0.025 (between 0.02 and 0.03)
    time_arr = wp.array([0.025], dtype=float)
    history.read_ctrl(m, d, 0, time_arr, interp=0, result=result)
    np.testing.assert_allclose(result.numpy()[0], 30.0, atol=_TOLERANCE, err_msg="t=0.025")
...
class SensorDelayTest(absltest.TestCase):
  ...
  def test_sensor_history_only(self):
    """Test sensor history with delay=0.

    delay=0.0, timestep=0.01, nsample=5.
    """
    xml = """
    <mujoco>
      <option timestep="0.01" gravity="0 0 0"/>
    ...
    sdata = d.sensordata.numpy()[0, 0]
    np.testing.assert_allclose(sdata, 10.0, atol=_TOLERANCE)

    time_arr = wp.array([d.time.numpy()[0] - 0.005], dtype=float)
    result = wp.empty((d.nworld, 1), dtype=float)
    history.read_sensor(m, d, 0, time_arr, interp=0, result=result)
...
class PublicAPITest(absltest.TestCase):
  ...
  def test_read_ctrl(self):
    ...
    history.read_ctrl(m, d, 0, time_arr, interp=-1, result=warp_result)
    ...
    history.read_ctrl(m, d, 0, time_arr, interp=0, result=warp_result_zoh)
  ...
  def test_read_sensor(self):
    ...
    history.read_sensor(m, d, 0, time_arr, interp=-1, result=result)
  ...
  def test_init_ctrl_history(self):
    ...
    history.init_ctrl_history(m, d, 0, times_wp, values_wp)
    ...
    warp_result = wp.empty(d.nworld, dtype=float)
    history.read_ctrl(m, d, 0, time_arr, interp=0, result=warp_result)
  ...
  def test_init_sensor_history(self):
    ...
    history.init_sensor_history(m, d, 0, times_wp, values_wp, phase=phase_wp)
