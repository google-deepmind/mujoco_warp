...
@wp.kernel
def _spring_damper_tendon_passive_det(
  # Model:
  tendon_stiffness: wp.array2d[float],
  tendon_stiffnesspoly: wp.array2d[wp.vec2],
  tendon_damping: wp.array2d[float],
  ...
  adr_beg = dof_ten_adr[dofid]
  adr_end = dof_ten_adr[dofid + 1]
  if adr_beg == adr_end:
    return
  ...
  # gather over the dof's tendons in ascending tendon id order
  for adr in range(adr_beg, adr_end):
    tenid = dof_ten_tenid[adr]
    ...
    J = ten_J_in[worldid, dof_ten_sparseid[adr]]
    ...
    if has_damping:
      ...
      # transform to joint torque
      qfrc_damper += J * frc_damper

  qfrc_spring_out[worldid, dofid] += qfrc_spring
...
def _gravity_force_det(
  ...
  adr_beg = dof_gravcomp_adr[dofid]
  adr_end = dof_gravcomp_adr[dofid + 1]
  if adr_beg == adr_end:
    return
  ...
  # gather over the dof's gravcomp bodies in ascending body id order
  for adr in range(adr_beg, adr_end):
    bodyid = dof_gravcomp_body[adr]
    gravcomp = body_gravcomp[worldid % body_gravcomp.shape[0], bodyid]
    ...
    if gravcomp != 0.0:
      ...
      qfrc += wp.dot(jac, force)

  qfrc_gravcomp_out[worldid, dofid] += qfrc
...
@cache_kernel
def _flex_elasticity(deterministic: bool):
  ...
  def kernel(
    ...
    flex_partial_spring_out: wp.array2d[wp.spatial_vector],
  ):
    ...
    for v in range(nvert):
      ...
      if wp.static(deterministic):
        flex_partial_spring_out[worldid, flex_partial_elas_base + elem_data_adr + v] = spatial_frc
      else:
...
@cache_kernel
def _flex_bending(deterministic: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    # Model:
    nflex: int,
    ...
    flex_partial_spring_out: wp.array2d[wp.spatial_vector],
    flex_partial_damper_out: wp.array2d[wp.spatial_vector],
  ):
    ...
    for i in range(4):
      ...
      if wp.static(deterministic):
        flex_partial_spring_out[worldid, flex_partial_bend_base + 4 * edgeid + i] = spatial_frc_s
      else:
      ...
      if not dsbl_damper and flex_damping[f] > 0.0:
        ...
        if wp.static(deterministic):
          flex_partial_damper_out[worldid, flex_partial_bend_base + 4 * edgeid + i] = spatial_frc_d
        else:
...
@cache_kernel
def _flex_passive_interp(deterministic: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    # Model:
    nflex: int,
        ...
        for lk_i in range(order + 1):
          if idx_i < npc:
            ...
            if not dsbl_spring:
              ...
              if wp.static(deterministic):
                flex_partial_spring_out[worldid, flex_partial_interp_base + cellid * flex_partial_interp_npc + idx_i] = (
                  spatial_frc_s
                )
              else:
            ...
            if not dsbl_damper:
              ...
              if wp.static(deterministic):
                flex_partial_damper_out[worldid, flex_partial_interp_base + cellid * flex_partial_interp_npc + idx_i] = (
                  spatial_frc_d
                )
              else:
...
def _apply_face_forces_det(
  ...
  slot_base: int,
  # Out:
  partial_out: wp.array2d[wp.spatial_vector],
):
  idx = int(0)
  for l0 in range(3):
    if l0 > order_abs:
    ...
    for l1 in range(3):
      ...
      offset = body_xipos - node_pos
      spatial_frc = wp.spatial_vector(frc, -wp.cross(offset, frc))
      partial_out[worldid, slot_base + idx] = spatial_frc
      idx += 1


@cache_kernel
def _flex_passive_bend_interp(deterministic: bool):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    # Model:
    nflex: int,
    ...
    if wp.static(deterministic):
      ...
      _apply_face_forces_det(
        ...
        order_abs,
        worldid,
        flex_partial_face_base + 18 * bend_edge_id,
        flex_partial_spring_out,
      )
      ...
      _apply_face_forces_det(
        ...
        order_abs,
        worldid,
        flex_partial_face_base + 18 * bend_edge_id + 9,
        flex_partial_spring_out,
      )
    else:
      # 4. Apply forces for Face A
...
@wp.kernel
def _flex_body_force_gather(
  # Model:
  flexbody_partial_adr: wp.array[int],
  flexbody_partialid: wp.array[int],
  # In:
  flex_partial_spring_in: wp.array2d[wp.spatial_vector],
  flex_partial_damper_in: wp.array2d[wp.spatial_vector],
  # Out:
  flex_spring_body_force_out: wp.array2d[wp.spatial_vector],
  flex_damper_body_force_out: wp.array2d[wp.spatial_vector],
):
  ...
  adr_beg = flexbody_partial_adr[bodyid]
  adr_end = flexbody_partial_adr[bodyid + 1]
  if adr_beg == adr_end:
    return

  frc_spring = wp.spatial_vector()
  frc_damper = wp.spatial_vector()

  # gather the body's partial forces in ascending slot id order
  for adr in range(adr_beg, adr_end):
    slotid = flexbody_partialid[adr]
    frc_spring += flex_partial_spring_in[worldid, slotid]
    frc_damper += flex_partial_damper_in[worldid, slotid]

  flex_spring_body_force_out[worldid, bodyid] += frc_spring
  flex_damper_body_force_out[worldid, bodyid] += frc_damper


def _ensure_det_flex_scratch(m: Model, d: Data) -> dict:
  """Lazily allocate persisted per-scatter-slot scratch for deterministic flex forces."""
  scratch = getattr(d, "_det_flex_scratch", None)
  if scratch is not None:
    return scratch

  scratch = {
    "partial_spring": wp.empty((d.nworld, m.nflexpartial), dtype=wp.spatial_vector),
    "partial_damper": wp.empty((d.nworld, m.nflexpartial), dtype=wp.spatial_vector),
  }
  d._det_flex_scratch = scratch
  return scratch
...
def passive(m: Model, d: Data):
  ...
  if m.ntendon:
    if det:
      wp.launch(
        _spring_damper_tendon_passive_det,
        dim=(d.nworld, m.nv),
        inputs=[
          m.tendon_stiffness,
  ...
  flex_partial_spring = None
  flex_partial_damper = None
  if m.nflex > 0:
    ...
    if det:
      flex_scratch = _ensure_det_flex_scratch(m, d)
      flex_partial_spring = flex_scratch["partial_spring"]
      flex_partial_damper = flex_scratch["partial_damper"]
      flex_partial_spring.zero_()
      flex_partial_damper.zero_()
    else:
      # dummy arrays for the deterministic-only kernel params in non-det mode
      flex_partial_spring = wp.zeros((1, 1), dtype=wp.spatial_vector, device=d.qfrc_spring.device)
      flex_partial_damper = flex_partial_spring

  if not dsbl_spring:
    wp.launch(
      _flex_elasticity(det),
      ...
      outputs=[flex_spring_body_force, flex_partial_spring],
    )
  ...
  if m.nflex and m.nflexintcell > 0:
    ...
    wp.launch(
      ...
      inputs=[
        ...
        m.flex_centered,
        m.flex_cell_map,
        m.flex_partial_interp_base,
        m.flex_partial_interp_npc,
        d.xipos,
        d.subtree_com,
        d.cvel,
        d.flexnode_xpos,
  ...
  if m.nflex > 0 and det:
    # combine the per-slot partial forces into per-body totals in a fixed order
    wp.launch(
      _flex_body_force_gather,
      dim=(d.nworld, m.nbody),
      inputs=[m.flexbody_partial_adr, m.flexbody_partialid, flex_partial_spring, flex_partial_damper],
      outputs=[flex_spring_body_force, flex_damper_body_force],
    )
