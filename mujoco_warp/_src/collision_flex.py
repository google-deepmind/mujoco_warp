...
def _write_flex_contact(
  ...
  if dist_in >= margin_in or dist_in >= MJ_MAXVAL:
    return
  ...
  contact_dist_out[id_] = dist_in
  contact_pos_out[id_] = pos_in
  contact_frame_out[id_] = frame_in
  contact_includemargin_out[id_] = margin_in
  contact_friction_out[id_] = friction_in
  contact_solref_out[id_] = solref_in
  contact_solreffriction_out[id_] = wp.vec2(0.0, 0.0)
  contact_solimp_out[id_] = solimp_in
  contact_dim_out[id_] = condim_in
...
def _collide_geom_triangle(
  ...
  if gtype == int(GeomType.SPHERE):
    ...
    dist, pos, nrm = collision_primitive_core.sphere_triangle(
        geom_pos, sphere_radius, t1, t2, t3, tri_radius
    )
    if dist < margin:
      _write_flex_contact(
          naconmax_in, dist, pos, make_frame(nrm), margin, condim, friction,
          solref, solimp, geomid, flexid, vertex_id, worldid, contact_dist_out,
          ...
          contact_type_out, contact_geomcollisionid_out, nacon_out
      )
    ...
    dists, poss, nrms = collision_primitive_core.box_triangle(
        geom_pos, geom_rot, geom_size_val, t1, t2, t3, tri_radius
    )
    if dists[0] < margin:
      p1 = wp.vec3(poss[0, 0], poss[0, 1], poss[0, 2])
      n1 = wp.vec3(nrms[0, 0], nrms[0, 1], nrms[0, 2])
...
def _flex_plane_narrowphase(
  ...
  for geomid in range(ngeom):
    ...
    margin = geom_margin[worldid, geomid] + flex_margin_val
    ...
    if dist < margin:
      ...
      solref = geom_solref[worldid, geomid]
      solimp = geom_solimp[worldid, geomid]
      fric = geom_friction[worldid, geomid]
      friction = vec5(fric[0], fric[1], fric[2], 0.0, 0.0)

      contact_pos = vert - plane_normal * (dist * 0.5 + radius)
      _write_flex_contact(
          naconmax_in, dist, contact_pos, make_frame(plane_normal), margin,
          condim, friction, solref, solimp, geomid, flexid, vertid, worldid,
          contact_dist_out, contact_pos_out, contact_frame_out,
          ...
          contact_geomcollisionid_out, nacon_out
      )
...
def _flex_narrowphase_dim2(
  ...
  for geomid in range(ngeom):
    ...
    solref = geom_solref[0, geomid]
    solimp = geom_solimp[0, geomid]

    _collide_geom_triangle(
        gtype, geom_pos, geom_rot, geom_size_val, t1, t2, t3, tri_radius,
        margin, condim, friction, solref, solimp, geomid, flexid, v0_local,
        worldid, naconmax_in, contact_dist_out, contact_pos_out, contact_frame_out,
        contact_includemargin_out, contact_friction_out, contact_solref_out,
        contact_solreffriction_out, contact_solimp_out, contact_dim_out,
        contact_geom_out, contact_flex_out, contact_vert_out,
        contact_worldid_out, contact_type_out,
