...
def _scalar_cholesky_solve(
  ...
  for i in range(block_size):
    value = y[worldid, vector_adr + i]
    for k in range(i):
      value -= L[worldid, factor_adr + k * block_size + i] * x_out[worldid, vector_adr + k]
    x_out[worldid, vector_adr + i] = value / L[worldid, factor_adr + i * block_size + i]

  # Backward substitution U * x = z, overwriting z with the solution.
  for reverse_i in range(block_size):
    i = block_size - 1 - reverse_i
    value = x_out[worldid, vector_adr + i]
    for k in range(i + 1, block_size):
      value -= L[worldid, factor_adr + i * block_size + k] * x_out[worldid, vector_adr + k]
    x_out[worldid, vector_adr + i] = value / L[worldid, factor_adr + i * block_size + i]
...
@cache_kernel
def _scalar_cholesky_solve_block_island(block_size: int):
  @wp.kernel(module="unique", enable_backward=False)
  def kernel(
    # Model:
    qLD_block_adr: wp.array[int],
