  ...
  for i in range(start, end):
    ...
    if jntnum >= 1:
      xpos, xquat = _process_joint(
        xpos, xquat, jntadr, jnt_pos_id, worldid,
        qpos0, jnt_type, jnt_qposadr, jnt_pos, jnt_axis, qpos,
        xanchor_out, xaxis_out)
    if jntnum >= 2:
      xpos, xquat = _process_joint(
        xpos, xquat, jntadr + 1, jnt_pos_id, worldid,
        qpos0, jnt_type, jnt_qposadr, jnt_pos, jnt_axis, qpos,
        xanchor_out, xaxis_out)
    if jntnum >= 3:
      xpos, xquat = _process_joint(
        xpos, xquat, jntadr + 2, jnt_pos_id, worldid,
        qpos0, jnt_type, jnt_qposadr, jnt_pos, jnt_axis, qpos,
        xanchor_out, xaxis_out)
    if jntnum >= 4:
      xpos, xquat = _process_joint(
        xpos, xquat, jntadr + 3, jnt_pos_id, worldid,
        qpos0, jnt_type, jnt_qposadr, jnt_pos, jnt_axis, qpos,
        xanchor_out, xaxis_out)
...
def _cacc_branch(
  ...
  for i in range(start, end):
    ...
    # produces incorrect gradients in warp's AD
    if dofnum >= 1:
      local_cacc = _process_dof_cacc(
        local_cacc, dofadr, worldid, qvel_in, qacc_in, cdof_in, cdof_dot_in, flg_acc)
    if dofnum >= 2:
      local_cacc = _process_dof_cacc(
        local_cacc, dofadr + 1, worldid, qvel_in, qacc_in, cdof_in, cdof_dot_in, flg_acc)
    if dofnum >= 3:
      local_cacc = _process_dof_cacc(
        local_cacc, dofadr + 2, worldid, qvel_in, qacc_in, cdof_in, cdof_dot_in, flg_acc)
    if dofnum >= 4:
      local_cacc = _process_dof_cacc(
        local_cacc, dofadr + 3, worldid, qvel_in, qacc_in, cdof_in, cdof_dot_in, flg_acc)
    if dofnum >= 5:
      local_cacc = _process_dof_cacc(
        local_cacc, dofadr + 4, worldid, qvel_in, qacc_in, cdof_in, cdof_dot_in, flg_acc)
    if dofnum >= 6:
      local_cacc = _process_dof_cacc(
        local_cacc, dofadr + 5, worldid, qvel_in, qacc_in, cdof_in, cdof_dot_in, flg_acc)
...
def _comvel_branch(
  ...
  for i in range(start, end):
    ...
    # produces incorrect gradients in warp's AD
    if jntnum >= 1:
      cvel, dofid = _process_joint_vel(
        cvel, dofid, jntid, worldid, jnt_type, qvel, cdof, cdof_dot_out)
    if jntnum >= 2:
      cvel, dofid = _process_joint_vel(
        cvel, dofid, jntid + 1, worldid, jnt_type, qvel, cdof, cdof_dot_out)
    if jntnum >= 3:
      cvel, dofid = _process_joint_vel(
        cvel, dofid, jntid + 2, worldid, jnt_type, qvel, cdof, cdof_dot_out)
    if jntnum >= 4:
      cvel, dofid = _process_joint_vel(
        cvel, dofid, jntid + 3, worldid, jnt_type, qvel, cdof, cdof_dot_out)
