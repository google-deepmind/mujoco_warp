...
[Differentiability via Warp](https://nvidia.github.io/warp/user_guide/differentiability.html) is available for a supported subset of the pipeline: create gradient-tracked data with `mjw.make_diff_data` (or `mjw.enable_grad`), record `mjw.step` (or `mjw.step1`/`mjw.step2`) calls on a `wp.Tape`, and read gradients of a scalar loss with respect to inputs such as `qpos`, `qvel`, `ctrl`, `act`, `qfrc_applied`, `xfrc_applied`, `mocap_pos`/`mocap_quat`, and smooth position/velocity-stage sensors. Supported scope: `EULER`/`IMPLICITFAST`/`RK4` integrators, the `NEWTON` solver (contact gradients flow through an implicit fixed-point backward of the converged solve), smooth dynamics including passive spring/damper, fluid, and flex elasticity/bending forces, delayed (`delay`/`nsample`) actuator controls, and differentiable contact geometry for primitive pairs (plane/sphere/capsule combinations). Not (yet) differentiable: the `IMPLICIT` integrator (stepping it under a tape raises `NotImplementedError`), the `CG` solver, contacts on box/mesh/hfield/sdf pairs and flex contacts (a runtime warning is emitted for models with flexes and contact enabled), `FLEX`/`FLEXSTRAIN` equality rows, contact-dependent and acceleration-stage sensors, and most physical model parameters (see the `enable_grad` docstring for the precise lists). CUDA graph capture of a step under an active tape raises an error (graph replay bypasses tape recording). See [#500](https://github.com/google-deepmind/mujoco_warp/issues/500) for progress.
